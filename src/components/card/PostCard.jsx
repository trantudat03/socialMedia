...
const CommentForm = ({ id, replyAt, getComments }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if (replyAt) {
      ...
      axios
        .post("/comment", { idPost: id, comment: data.comment, from: name })
        .then((res) => {
          getComments();
          reset();
...
const PostCard = ({ post, deletePost, likePost, handDetail }) => {
  ...
  const getComments = async (id) => {
    ...
    axios
      ...
      .then(async (res) => {
        ...
        setCmtLength(res.data.data.length);
      })
      .catch(() => {
        console.log("error");
      });
  ...
  const handShowReply = (cmtId) => {
    if (showReply.length !== 0) {
      const reply = showReply.find((reply) => reply === cmtId);

      if (reply) {
        setShowReply(showReply.filter((reply) => reply !== cmtId));
      } else {
        setShowReply([...showReply, cmtId]);
      }
    } else {
      setShowReply([cmtId]);
    }
                ...
                <div className="py-2 px-8 mt-6">
                  {comment?.replies?.length > 0 && (
                    <p
                      ...
                      onClick={() => handShowReply(comment?._id)}
                    >
                      Show Replies ({comment?.replies?.length})
                    </p>
                  )}

                  {showReply.indexOf(comment?._id) !== -1 &&
